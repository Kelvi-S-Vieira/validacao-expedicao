...
import {
  ...
  Trash2, ShieldCheck, Save, ChevronDown
} from 'lucide-react'
...
function SeletorSessoes({ sessoes, sessaoId, onSelecionar, onNovaUpload }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {aberto && (
        <div style={{
          position: 'absolute', top: 44, left: 0, zIndex: 100, minWidth: 280,
          background: 'var(--bg-card)', border: '1px solid var(--border)',
            ...
            return (
              <div key={s.id} onClick={() => { onSelecionar(s.id); setAberto(false) }}
                style={{
                  ...
                  background: ativa ? 'var(--yellow-dim)' : 'transparent',
...
export default function EntradaDados({ usuario, conferentes = [], onDadosSalvos }) {
  ...
  async function handleFinalizar() {
    ...
    try {
      await finalizarDoca(docaAtiva, {
        status: novoStatus, hrFim: agora, hrValidado: agora, valDoca: tempo,
  ...
  async function salvarNoSheets() {
    ...
    try {
      ...
      const linhas = docasSalvar.map(d => [
        ...
        d.valDoca || '', d.pendExp || 0, d.pendFrente || 0, d.pendDentro || 0,
    ...
    return (
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '0 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          {onCancelar && (
            <button onClick={onCancelar} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 8, padding: '7px 12px', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 12 }}>
              ← Voltar
            </button>
          )}
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
              Upload do arquivo de composição — será salvo junto com as sessões anteriores.
            </p>
          </div>
        </div>
        <div
  ...
  // ─── SEM SESSÕES — só fiscal vê upload ───────────────────
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>Iniciado às {docaObj?.hrInicio} · {sessao?.data}</p>
          </div>
        </div>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: 14, border: '1px solid var(--border)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '16px' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <Campo label="Observação" valor={valLocal.observacao} onChange={v => setValLocal(p => ({ ...p, observacao: v }))} type="text" placeholder={valLocal.cargaCertificada ? 'Ex: volume extra, divergência...' : 'Opcional...'} />
              {valLocal.cargaCertificada && (
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>
                  Será salvo como: <strong style={{ color: 'var(--green)' }}>CARGA CERTIFICADA{valLocal.observacao ? ` - ${valLocal.observacao}` : ''}</strong>
                </div>
              )}
            </div>
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 0 80px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {ehFiscal && sessao && (
            <button onClick={() => { if (confirm('Encerrar esta sessão e limpar suas docas?')) encerrarSessao() }}
              ...
              <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 150 }}>
                  ...
                  <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                    ...
                    {d.resultado !== null && d.resultado !== undefined && <span style={{ fontSize: 11, fontWeight: 700, color: d.resultado >= 5 ? 'var(--red)' : d.resultado >= 3 ? 'var(--yellow)' : 'var(--green)' }}>Resultado: {d.resultado}</span>}
                  </div>
                </div>
