...
export default function EntradaDados({ usuario, conferentes = [], onDadosSalvos }) {
  ...
  if (carregando) return (
    <div style={{ textAlign: 'center', padding: 48, color: 'var(--text-muted)' }}>
      ...
      <div style={{ fontSize: 13 }}>Verificando sessão do dia...</div>
    </div>
  )
  ...
  if (tela === 'sucesso') return (
    <div style={{ maxWidth: 480, margin: '64px auto', textAlign: 'center', padding: '0 16px' }}>
      ...
      <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 32, lineHeight: 1.6 }}>
        Docas registradas no Google Sheets com sucesso.
      </p>
      <button onClick={() => setTela('lista')} style={{ background: 'var(--yellow)', border: 'none', borderRadius: 10, padding: '14px 32px', cursor: 'pointer', fontSize: 14, fontWeight: 800, color: '#1a1a1a' }}>
        Voltar
      </button>
    </div>
  )
    ...
    return (
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '0 16px' }}>
        <div style={{ marginBottom: 24 }}>
          <h2 style={{ fontSize: 20, fontWeight: 800, marginBottom: 8 }}>Iniciar sessão do dia</h2>
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
            Faça upload do arquivo <strong style={{ color: 'var(--yellow)' }}>Composição de Horários de Expedição</strong>.
            Todos os fiscais poderão ver as docas para validar.
          </p>
        </div>
        ...
        >
          ...
          {processando ? (
            <>
              <div style={{ width: 36, height: 36, border: '3px solid var(--border)', borderTopColor: 'var(--yellow)', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 14px' }} />
              <div style={{ fontSize: 14, color: 'var(--yellow)' }}>Processando arquivo...</div>
            </>
          ) : (
            <>
              <FileSpreadsheet size={48} color={dragOver ? 'var(--yellow)' : 'var(--text-muted)'} style={{ marginBottom: 14 }} />
              <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 6, color: dragOver ? 'var(--yellow)' : 'var(--text-primary)' }}>
                Arraste ou clique para selecionar
              </div>
              <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Aceita: .xlsm, .xlsx — aba "121"</div>
            </>
          )}
        </div>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>Iniciado às {docaObj?.hrInicio}</p>
          </div>
        </div>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: 14, border: '1px solid var(--border)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '16px' }}>
                  ...
                  {confsAtivos.map(c => (
                    <button key={c.id} onClick={() => setValLocal(p => ({ ...p, conferente: c.nome }))}
                      ...
                      style={{
                        ...
                        border: `1px solid ${valLocal.conferente === c.nome ? 'var(--yellow)' : 'var(--border)'}`,
                        background: valLocal.conferente === c.nome ? 'var(--yellow)' : 'var(--bg-secondary)',
                        color: valLocal.conferente === c.nome ? '#1a1a1a' : 'var(--text-secondary)',
                        cursor: 'pointer', fontSize: 13, fontWeight: valLocal.conferente === c.nome ? 700 : 400,
                        opacity: docaObj?.status === 'EM_ANDAMENTO' && docaObj?.conferente && docaObj?.conferente !== c.nome ? 0.3 : 1,
                      }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 12 }}>
              <div>
                <Campo label="Pend. Expedição" valor={valLocal.pendExp} onChange={v => setValLocal(p => ({ ...p, pendExp: v }))} required />
                <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 3 }}>Total na doca</div>
              </div>
              <div>
                <Campo label="Frente Doca" valor={valLocal.pendFrente} onChange={v => setValLocal(p => ({ ...p, pendFrente: v }))} required />
                <div style={{ fontSize: 10, color: 'var(--green)', marginTop: 3 }}>Localizado frente</div>
              </div>
              <div>
                <Campo label="Dentro Doca" valor={valLocal.pendDentro} onChange={v => setValLocal(p => ({ ...p, pendDentro: v }))} required />
              ...
              <div style={{ background: 'var(--bg-secondary)', borderRadius: 10, padding: '12px 16px', marginBottom: 12, border: `1px solid ${corRes}44` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                  ...
                  {resultado >= 3 && resultado < 5 && (
                    <div style={{ background: 'var(--orange-dim)', border: '1px solid var(--orange)', borderRadius: 8, padding: '5px 10px' }}>
                      <div style={{ fontSize: 11, color: 'var(--orange)', fontWeight: 700 }}>⚠️ Requer coordenador (5 min → gerente)</div>
                    </div>
                  )}
                  {resultado >= 5 && (
                    <div style={{ background: 'var(--red-dim)', border: '1px solid var(--red)', borderRadius: 8, padding: '5px 10px' }}>
                      <div style={{ fontSize: 11, color: 'var(--red)', fontWeight: 700 }}>🚨 Requer gerente</div>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 10 }}>
              ...
              {!preenchido && (
                <button disabled style={{ padding: '14px', borderRadius: 10, border: 'none', background: 'var(--border)', color: 'var(--text-muted)', fontWeight: 800, fontSize: 15, cursor: 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 0 80px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {ehFiscal && (
            <button onClick={() => { if (confirm('Encerrar sessão e limpar todas as docas?')) encerrarSessao() }}
          ...
          {prontas.length > 0 && (ehFiscal || usuario?.perfil === 'COORDENADOR' || usuario?.perfil === 'GERENTE') && (
      ...
      {!docaAtiva && (
        <div style={{ background: 'var(--bg-card)', borderRadius: 12, padding: '14px 16px', border: '1px solid var(--border)', marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {confsAtivos.map(c => (
              <button key={c.id} onClick={() => setValLocal(p => ({ ...p, conferente: c.nome }))} style={{
                padding: '8px 14px', borderRadius: 20,
                border: `1px solid ${valLocal.conferente === c.nome ? 'var(--yellow)' : 'var(--border)'}`,
                background: valLocal.conferente === c.nome ? 'var(--yellow)' : 'var(--bg-secondary)',
                color: valLocal.conferente === c.nome ? '#1a1a1a' : 'var(--text-secondary)',
              ...
              <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 150 }}>
                  ...
                  <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                    ...
                    {d.resultado !== null && d.resultado !== undefined && (
                      <span style={{ fontSize: 11, fontWeight: 700, color: d.resultado >= 5 ? 'var(--red)' : d.resultado >= 3 ? 'var(--yellow)' : 'var(--green)' }}>
                ...
                {d.status === 'EM_ANDAMENTO' && minha && (
                  <button onClick={() => handleContinuar(d)} style={{ background: 'var(--yellow-dim)', border: '1px solid var(--yellow)', borderRadius: 8, padding: '8px 14px', cursor: 'pointer', color: 'var(--yellow)', fontWeight: 700, fontSize: 12, flexShrink: 0 }}>
                    Continuar →
                  </button>
                )}
                {d.status === 'EM_ANDAMENTO' && !minha && (
                  <span style={{ fontSize: 11, color: 'var(--text-muted)', fontStyle: 'italic' }}>Em validação</span>
                )}
                {d.status === 'CONCLUIDO' && <CheckCircle size={18} color="var(--green)" />}
                {(d.status === 'ESCALADO' || d.status === 'AGUARD_GERENTE') && <Lock size={16} color="var(--red)" />}
              </div>
            </div>
          )
        })}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'var(--bg-card)', borderRadius: 16, padding: 24, maxWidth: 500, width: '100%', maxHeight: '80vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 20 }}>
              <button onClick={() => setTela('lista')} style={{ padding: '12px', borderRadius: 10, border: '1px solid var(--border)', background: 'none', cursor: 'pointer', color: 'var(--text-primary)', fontWeight: 600 }}>
                Voltar
              </button>
