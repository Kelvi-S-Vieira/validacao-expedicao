...
const LABELS = {
  entrada: 'Entrada de Dados', dashboard: 'Dashboard',
  historico: 'Histórico 2025', tabela: 'Tabela de Docas',
  alertas: 'Alertas', aprovacao: 'Aprovação',
  relatorio: 'Relatório', conferentes: 'Conferentes',
}

const ICONS = {
  entrada:     <PlusCircle size={14} />,
  dashboard:   <BarChart2 size={14} />,
  historico:   <History size={14} />,
  tabela:      <ClipboardList size={14} />,
  alertas:     <AlertCircle size={14} />,
  aprovacao:   <CheckCircle size={14} />,
  relatorio:   <FileText size={14} />,
  conferentes: <Users size={14} />,
}

function MetricCard({ label, valor, icon, cor, sub }) {
  return (
    <div style={{ background: 'var(--bg-card)', borderRadius: 14, padding: '20px 24px', border: '1px solid var(--border)', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: cor, borderRadius: '14px 14px 0 0' }} />
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: 600 }}>{label}</div>
          <div style={{ fontSize: 36, fontWeight: 900, color: cor, lineHeight: 1 }}>{valor}</div>
          {sub && <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 8 }}>{sub}</div>}
        </div>
        <div style={{ background: cor + '20', borderRadius: 10, padding: 10, color: cor }}>{icon}</div>
      </div>
    </div>
  )
}
...
function Badge({ text }) {
  ...
  return (
    <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700, background: c.bg, color: c.color, whiteSpace: 'nowrap' }}>
      {text || 'N/A'}
    </span>
  )
}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontFamily: 'Inter, sans-serif' }}>
      ...
      {/* HEADER */}
      <div style={{ background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)', padding: '0 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 60 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ background: 'var(--yellow)', borderRadius: 8, padding: '4px 10px' }}>
            <span style={{ fontSize: 12, fontWeight: 900, color: '#1a1a1a' }}>grupo<strong>pernambucanas</strong></span>
          </div>
          <div style={{ width: 1, height: 24, background: 'var(--border)' }} />
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Truck size={16} color="var(--yellow)" />
            <span style={{ fontSize: 14, fontWeight: 700 }}>Validação de Expedição</span>
          </div>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {ultimaAtualizacao && <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Atualizado {ultimaAtualizacao}</span>}
      ...
      {/* NAVEGAÇÃO */}
      <div style={{ background: 'var(--bg-secondary)', padding: '0 32px', display: 'flex', gap: 2, borderBottom: '1px solid var(--border)', overflowX: 'auto' }}>
        {abasPerfil.map(aba => (
          <button key={aba} onClick={() => setAbaSelecionada(aba)} style={{
            ...
            fontWeight: abaSelecionada === aba ? 700 : 400, fontSize: 13,
            display: 'flex', alignItems: 'center', gap: 6,
          }}>
      ...
      <div style={{ padding: '24px 32px' }}>
        ...
        {carregando && !semFiltro.includes(abaSelecionada) && (
          <div style={{ textAlign: 'center', padding: 64, color: 'var(--text-muted)' }}>
            <div style={{ width: 40, height: 40, border: '3px solid var(--border)', borderTopColor: 'var(--yellow)', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 16px' }} />
          ...
          {!semFiltro.includes(abaSelecionada) && !carregando && !erro && (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
                ...
                <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Período:</span>
              ...
              {abaSelecionada === 'dashboard' && (
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
                    <MetricCard label="Total de Docas"     valor={totalDocas}     icon={<Package size={20} />}       cor="#3b82f6" sub={`${dias.filter(d => d !== 'todos').length} dia(s)`} />
                    <MetricCard label="Docas Liberadas"    valor={totalLiberadas} icon={<CheckCircle size={20} />}   cor="#22c55e" sub={`${pctLiberadas}% do total`} />
                    <MetricCard label="Alertas Ativos"     valor={totalAlertas}   icon={<AlertTriangle size={20} />} cor="var(--yellow)" sub="Aguardando ação" />
                    <MetricCard label="Críticos (Gerente)" valor={totalCriticos}  icon={<TrendingUp size={20} />}    cor="#ef4444" sub="Resultado ≥ 5" />
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
                    <div style={{ background: 'var(--bg-card)', borderRadius: 14, padding: 24, border: '1px solid var(--border)' }}>
                      <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 20, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Evolução Diária</div>
                    ...
                    <div style={{ background: 'var(--bg-card)', borderRadius: 14, padding: 24, border: '1px solid var(--border)' }}>
                      <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 20, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Status Geral</div>
                      <ResponsiveContainer width="100%" height={160}>
                        <PieChart>
                          <Pie data={dadosPizza} cx="50%" cy="50%" innerRadius={50} outerRadius={75} dataKey="value" paddingAngle={3}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                      <thead>
                        <tr style={{ background: 'var(--bg-secondary)' }}>
                          {['Data', 'Doca', 'Remessa', 'Horário', 'Pend.Exp', 'Frente', 'Dentro', 'Resultado', 'Supervisor', 'Turno', 'Liberado', 'Alerta'].map(col => (
                            <th key={col} style={{ padding: '12px 16px', textAlign: 'left', color: 'var(--text-muted)', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.5px', whiteSpace: 'nowrap' }}>{col}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {dadosFiltrados.map((row, i) => (
                          ...
                          >
                            <td style={{ padding: '12px 16px', color: 'var(--text-secondary)', fontSize: 12 }}>{row.data}</td>
                            <td style={{ padding: '12px 16px', fontWeight: 800, color: 'var(--yellow)' }}>{row.doca}</td>
                            <td style={{ padding: '12px 16px', fontFamily: 'monospace', fontSize: 12 }}>{row.remessa}</td>
                            <td style={{ padding: '12px 16px', fontSize: 12 }}><span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Clock size={11} />{row.horario}</span></td>
                            <td style={{ padding: '12px 16px', fontWeight: 700, color: row.pendExp > 0 ? 'var(--red)' : 'var(--text-muted)' }}>{row.pendExp}</td>
                            <td style={{ padding: '12px 16px', color: row.pendFrente > 0 ? 'var(--green)' : 'var(--text-muted)' }}>{row.pendFrente}</td>
                            <td style={{ padding: '12px 16px', color: row.pendDentro > 0 ? 'var(--green)' : 'var(--text-muted)' }}>{row.pendDentro}</td>
                            <td style={{ padding: '12px 16px', fontWeight: 900, color: row.resultado >= 5 ? 'var(--red)' : row.resultado >= 3 ? 'var(--yellow)' : 'var(--green)' }}>{row.resultado ?? row.total}</td>
                            <td style={{ padding: '12px 16px', fontSize: 12 }}>{row.supervisor}</td>
                            <td style={{ padding: '12px 16px', fontSize: 12 }}>{row.turno}</td>
                            <td style={{ padding: '12px 16px' }}><Badge text={row.liberado || 'N/A'} /></td>
                            <td style={{ padding: '12px 16px' }}>{row.alerta && <Badge text={row.alerta} />}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {dadosFiltrados.filter(d => d.alerta !== '').length === 0 ? (
                    <div style={{ textAlign: 'center', padding: 64, color: 'var(--text-muted)' }}>
                      ...
                      <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 8 }}>Tudo em ordem!</div>
                    ...
                    return (
                      <div key={i} style={{ background: 'var(--bg-card)', borderRadius: 12, padding: '16px 20px', border: `1px solid ${row.alerta === 'GERENTE' ? '#ef444430' : '#F5E64230'}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                          <div style={{ background: cor + '20', borderRadius: 10, padding: 10 }}>
                            <AlertTriangle size={20} color={cor} />
                          </div>
                          <div>
                            <div style={{ fontWeight: 800, fontSize: 15 }}>Doca {row.doca} — {row.remessa}</div>
              ...
              {abaSelecionada === 'aprovacao' && (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
                    ...
                    <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Visualizando como:</span>
